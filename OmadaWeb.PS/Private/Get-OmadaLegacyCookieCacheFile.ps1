function Get-OmadaLegacyCookieCacheFile {
    ...
    if ([string]::IsNullOrWhiteSpace($Script:LegacyCookieCachePath) -or -not (Test-Path -LiteralPath $Script:LegacyCookieCachePath -PathType Container)) {
        return @()
    }
    ...
    return @(Get-ChildItem -LiteralPath $Script:LegacyCookieCachePath -File -Force -ErrorAction SilentlyContinue |
