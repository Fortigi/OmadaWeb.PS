function Get-OmadaWebCacheItem {
    ...
    if ($SelectedScopes -contains "Cookies") {
        $Items.Add((New-OmadaWebCacheItem -Scope "Cookies" -Artefact "Encrypted cookie cache" -Path $Script:CookieCachePath -ItemType "Directory" -Protection "Encrypted with DPAPI for the current Windows user account"))
        ...
        if ($LegacyFiles.Count -gt 0) {
            $Items.Add((New-OmadaWebCacheItem -Scope "Cookies" -Artefact "Encrypted cookie cache (legacy %TEMP% location)" -Path $Script:LegacyCookieCachePath -ItemType "File" -Protection "Encrypted with DPAPI for the current Windows user account" -File $LegacyFiles))
        }
    }
